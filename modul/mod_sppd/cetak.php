...
<div id="wrapper">
	...
	<div id="cop">
		...
		Jln. Jendral Soedirman No.40 Lb.sikaping Pos 26311 Telp - (0753) 20202 - 20281
	</div>
	<hr>
...
$qry=mysqli_query($link, "SELECT * FROM sppd,nppt,pegawai,tujuan,jabatan,pangkat WHERE id_sppd='$_GET[id]' 
AND sppd.id_pegawai=pegawai.id_pegawai 
AND sppd.id_nppt=nppt.id_nppt 
AND nppt.id_tujuan=tujuan.id_tujuan 
AND jabatan.id_jabatan=pegawai.id_jabatan 
AND pangkat.id_pangkat=pangkat.id_pangkat");
$r=mysqli_fetch_array($qry);
$query_perintah = mysqli_query($link, "SELECT detail_nppt.id_detail,detail_nppt.id_nppt,detail_nppt.id_pegawai, pegawai.nip, pegawai.nama, pangkat.pangkat, jabatan.jabatan FROM detail_nppt 
...
$pegawai_perintah = mysqli_fetch_array($query_perintah);
?>
...
$no_pengikut = 1;
$pengikut = mysqli_query($link, "SELECT detail_nppt.id_detail,detail_nppt.id_nppt,detail_nppt.id_pegawai, pegawai.nama, pangkat.pangkat, jabatan.jabatan FROM detail_nppt 
	JOIN pegawai ON detail_nppt.id_pegawai = pegawai.id_pegawai 
	JOIN pangkat ON pegawai.id_pangkat = pangkat.id_pangkat 
...
while($data_pengikut = mysqli_fetch_array($pengikut)) {
...
?>
	<div style="width:300px;float:right;margin-top:15px"><span style="width:300px;float:right;">Dikeluarkan di: Lubuk Sikaping</span><br>
<?php
echo" <tr><td>Pada Tanggal	</td><td>: $tgldibuat</td></tr>";
?>
		<div style="font-weight:bold"><p align="center">Kepala Dinas Komunikasi dan Informatika<br>
			</p>
			<p>&nbsp;</p>
			<p align="center"><u>WILLIYAM HUTABARAT, S.Kom.</u><br>
			(Pembina TK. I)<br/>
